...
try {
    ...
    console.error(
        "You need to create a file called config.json and add your session cookie in a key called 'key'"
    );
    return;
}
...
if (!process.argv[2]) {
    ...
    if (year < 2015 || year > currentYear || day < 1 || day > 25) {
        console.error(
            "Invalid day or year provided. Only enter the numbers ex. 12 or 2018/12"
        );
        return;
    }
}
...
if (process.argv[3]) {
    ...
    console.warn(
        `No file name specified. File will be created as input-${day}.txt`
    );
